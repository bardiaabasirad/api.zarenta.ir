...
let inFlightCleanupTimer = setInterval(() => {
    const now = Date.now();
...
class CardScraperService {
    ...
    /**
     * 🧹 پاک‌سازی درخواست‌های در حال پردازش (recovery)
     */
    clearInFlight() {
        clearInFlight();
    }
...
// 🧹 ریست state ها (module-level, نیازی به instance نیست)
function resetState() {
    lastSentRates = {};
    lastSentUniqueKey = {};
...
// 🧹 پاک‌سازی درخواست‌های در حال پردازش (برای مسیر recovery)
