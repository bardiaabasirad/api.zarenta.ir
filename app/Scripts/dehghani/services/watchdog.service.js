...
const CardScraperService = require("./card-scraper.service");
const {resyncMonitoring} = require("../rate-monitor");
...
class WatchdogService {
    ...
    async checkHealth() {
        ...
        try {
            ...
            if (noItemContainer) {
                ...
                // از سرگیری واقعی اسکرپ/ارسال
                try {
                    await resyncMonitoring(this.page);
                } catch (e) {
                    logger.log(`Resync after items-back failed: ${e.message}`);
                }
            }
        ...
        try {
            ...
            if (!hasHook) {
                try {
                    ...
                    // already exposed in this browser context, safe to ignore
                }
            }
    ...
    async handleFailure() {
        ...
        try {
            // اگر مرورگر قطع است، اصلاً تلاش login بی‌فایده است → مستقیم ری‌استارت
            if (!this.page || this.page.isClosed?.() || !this.page.browser()?.isConnected()) {
                logger.log("Page/Browser dead → forcing restart");
            ...
            // NEW: پاک‌سازی state dedupe تا نرخ‌های بازگشته فیلتر نشوند
            CardScraperService.resetState();
            CardScraperService.clearInFlight();
        } catch (err) {
            this.recoveryAttempts++;
            logger.log(`Recovery failed (${this.recoveryAttempts}/${this.maxRecoveryAttempts}): ${err.message}`);

            if (this.recoveryAttempts >= this.maxRecoveryAttempts) {
                logger.log("Max recovery attempts reached → restarting application");
                this.terminate();
            }
        }
    }
